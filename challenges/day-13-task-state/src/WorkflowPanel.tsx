...
export default function WorkflowPanel({ workflow, disabled, modelUnavailable, mode, onModeChange, onAction, onGenerate }: Props) {
  ...
  const [artifact, setArtifact] = useState(workflow.artifact);
  useEffect(() => { setStep(workflow.step); setExpectedAction(workflow.expectedAction); }, [workflow.step, workflow.expectedAction]);
  useEffect(() => setPlan(workflow.plan), [workflow.plan]);
  useEffect(() => setArtifact(workflow.artifact), [workflow.artifact]);
  ...
  return <section className={`panel workflow-panel ${workflow.paused ? 'workflow-paused' : ''}`} id="workflow" aria-labelledby="workflow-heading">
    <div className="workflow-heading">
      <div><span className="eyebrow teal-text">КОНЕЧНЫЙ АВТОМАТ ЗАДАЧИ</span><h2 id="workflow-heading">Всегда понятно, где остановились</h2></div>
    ...
    <div className="workflow-controls">
      ...
      <div className="workflow-transitions">
        {workflow.stage === 'validation' && <button type="button" className="button button-quiet" disabled={locked} onClick={() => void onAction({ type: 'transition', target: 'execution' })}>← На доработку</button>}
        {next && <button type="button" className="button button-primary" disabled={locked} onClick={() => void onAction({ type: 'transition', target: next.id })}>{next.id === 'done' ? 'Завершить задачу' : `К этапу «${next.title}»`}<span aria-hidden="true">→</span></button>}
    ...
    <div className="workflow-materials">
      <details className="workflow-material" open={workflow.stage === 'planning'}>
        ...
        <div className="workflow-material-body"><label htmlFor="workflow-plan">Что будем делать и в каком порядке</label><textarea id="workflow-plan" value={plan} onChange={event => setPlan(event.target.value)} disabled={locked || workflow.stage !== 'planning'} rows={6} maxLength={12000} placeholder="Запишите план или попросите агента подготовить его из памяти задачи." /><div className="workflow-material-actions"><button className="button button-secondary" disabled={locked || workflow.stage !== 'planning' || plan === workflow.plan || !plan.trim()} onClick={() => void onAction({ type: 'set-plan', value: plan.trim() })}>Сохранить план</button><button className="button button-primary" disabled={generateDisabled || workflow.stage !== 'planning'} onClick={() => onGenerate('plan')}>Создать план с агентом</button></div><p>{workflow.stage !== 'planning' ? 'План доступен для чтения. Создание и изменение — на этапе планирования.' : mode === 'live' ? 'Генерация — один платный запрос. План сохранится отдельно от диалога.' : 'Генерация — одна симуляция. План сохранится отдельно от диалога.'}</p></div>
      </details>
      <details className="workflow-material" open={workflow.stage === 'execution'}>
        ...
        <div className="workflow-material-body"><label htmlFor="workflow-artifact">Содержание подготовленного решения</label><textarea id="workflow-artifact" value={artifact} onChange={event => setArtifact(event.target.value)} disabled={locked || workflow.stage !== 'execution'} rows={7} maxLength={16000} placeholder="Сохраните результат выполнения плана или создайте его с агентом." /><div className="workflow-material-actions"><button className="button button-secondary" disabled={locked || workflow.stage !== 'execution' || artifact === workflow.artifact || !artifact.trim()} onClick={() => void onAction({ type: 'set-artifact', value: artifact.trim() })}>Сохранить результат</button><button className="button button-primary" disabled={generateDisabled || workflow.stage !== 'execution'} onClick={() => onGenerate('artifact')}>Выполнить с агентом</button></div><p>{workflow.stage !== 'execution' ? 'Результат доступен для чтения. Изменение — на этапе выполнения.' : mode === 'live' ? 'Генерация — один платный запрос. Результат сохранится в задаче.' : 'Генерация — одна симуляция. Результат сохранится в задаче.'}</p></div>
      </details>
      <details className="workflow-material" open={workflow.stage === 'validation' || workflow.stage === 'done'}>
        <summary><span className="material-number">03</span><strong>Проверка результата</strong><span className="material-version">{workflow.review ? `Для версии ${workflow.review.artifactVersion}` : 'Ещё не выполнена'}</span></summary>
        <div className="workflow-material-body">{workflow.review ? <><span className={`status-badge ${workflow.review.mode === 'demo' ? 'status-demo' : ''}`}>{workflow.review.mode === 'demo' ? 'Симуляция проверки' : 'Проверено моделью'}</span><div className="workflow-review">{workflow.review.text}</div>{workflow.review.artifactVersion !== workflow.artifactVersion && <p className="workflow-stale-review">Результат изменён. Эта проверка относится к другой версии.</p>}</> : <div className="workflow-review-empty">Агент сопоставит сохранённый результат с планом и данными задачи. После проверки можно вернуться к выполнению или завершить задачу.</div>}<div className="workflow-material-actions"><button className="button button-primary" disabled={generateDisabled || workflow.stage !== 'validation'} onClick={() => onGenerate('review')}>{workflow.review ? 'Повторить проверку' : 'Проверить с агентом'}</button></div><p>{workflow.stage !== 'validation' ? 'Проверка запускается на этапе «Проверка».' : mode === 'live' ? 'Один платный запрос. Прочитайте вывод агента перед следующим переходом.' : 'Одна симуляция без API; она не оценивает качество реальной моделью.'}</p></div>
      </details>
    </div>
  </section>;
}
