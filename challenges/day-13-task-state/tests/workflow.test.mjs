...
const answer = async () => ({ text: 'Сохранённый результат тестового провайдера.', model: 'test-provider', usage: null });
...
test('пауза на каждом этапе сохраняет шаг, запрещает работу и восстанавливается после перезапуска', async (t) => {
  const directory = await mkdtemp(join(tmpdir(), 'day13-pause-test-'));
...
test('состояние задачи присутствует в запросе без истории и всех слоёв памяти', () => {
  let state = createInitialState();
  state = applyAction(state, { type: 'set-plan', value: 'SAVED_PLAN_FOR_CONTEXT_614e' });
  state = transition(state, 'execution');
  state = applyAction(state, { type: 'set-artifact', value: 'SAVED_ARTIFACT_FOR_CONTEXT_6d1a' });
  ...
  const stateBlock = context.input.find((message) => message.role === 'developer' && message.content.includes('SAVED_PLAN_FOR_CONTEXT_614e'));
  assert.ok(stateBlock);
  for (const marker of ['execution', 'SAVED_ARTIFACT_FOR_CONTEXT_6d1a', 'CURRENT_STEP_MARKER_2d90', 'NEXT_ACTION_MARKER_900e']) assert.ok(stateBlock.content.includes(marker));
...
test('новый диалог сохраняет состояние задачи, новая задача и профиль получают независимое состояние', () => {
  let state = createInitialState();
  state = applyAction(state, { type: 'set-plan', value: 'FIRST_TASK_PLAN_6b71' });
  state = transition(state, 'execution');
  ...
  assert.ok(!JSON.stringify(buildContext(otherProfile, 'Что дальше?', [])).includes('FIRST_TASK_PLAN_6b71'));
  assert.ok(!JSON.stringify(view(otherProfile)).includes('FIRST_TASK_PLAN_6b71'));
...
test('генерация плана, результата и проверки сохраняет версии без добавления в переписку', async () => {
  ...
  for (const [kind, stage, field] of [['plan', 'planning', 'plan'], ['artifact', 'execution', 'artifact'], ['review', 'validation', 'review']]) {
    if (workflow(state).stage !== stage) state = transition(state, stage);
    const before = structuredClone(state);
    const text = `GENERATED_${kind}_57e8`;
    const generated = await generateWorkflow(state, { kind, mode: 'live' }, async () => ({ ...(await answer()), text }));
    assert.deepEqual(state, before);
...
test('план и результат сохраняются в отдельном task-state.json и читаются после перезапуска', async (t) => {
  const directory = await mkdtemp(join(tmpdir(), 'day13-workflow-store-test-'));
  t.after(() => rm(directory, { recursive: true, force: true }));
  let state = applyAction(createInitialState(), { type: 'set-plan', value: 'SEPARATE_PLAN_FILE_922d' });
  state = transition(state, 'execution');
  state = applyAction(state, { type: 'set-artifact', value: 'SEPARATE_ARTIFACT_FILE_c62e' });
  new Repository(directory).write(state);
  ...
  for (const name of ['sessions.json', 'tasks.json', 'long-term.json', 'system.json']) {
    ...
    assert.ok(!raw.includes('SEPARATE_PLAN_FILE_922d'));
    assert.ok(!raw.includes('SEPARATE_ARTIFACT_FILE_c62e'));
  }
