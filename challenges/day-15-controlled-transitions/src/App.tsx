...
import type { Workflow, Gate, Gates } from './WorkflowPanel';
import InvariantsPanel from './InvariantsPanel';
import type { Invariants, Policy } from './InvariantsPanel';

type Layer = 'short' | 'work' | 'long';
type Mode = 'live' | 'demo';
type ResultMode = Mode | 'validation';
...
type Result = { text: string; usage?: Usage | null; context?: Context; mode?: ResultMode; label?: string; policy?: Policy; gate?: Gate };
type AppState = {
  day: number;
  title: string;
  revision: number;
...
function UsageLine({ usage, mode }: { usage?: Usage | null; mode?: ResultMode }) {
  if (mode === 'validation') return <span className="usage-line">Проверка правил · без вызова модели</span>;
...
export default function App() {
  ...
  async function run(label: string, operation: () => Promise<void>, scope = '') {
    if (pending) return;
    setPending(label);
    setError('');
    setErrorScope(scope);
  ...
  async function checkProposal(changes: Record<string, string | number>) {
    if (busy) return;
    await run('Проверяем предложение по правилам', async () => {
      const response = await request<{ state: AppState; result: Result }>('/api/proposal', { changes });
      setState(response.state);
      setPreview(null);
    });
  }
  ...
  function fillMessage(value: string) {
    setMessage(value);
    setPreview(null);
    const input = document.querySelector<HTMLTextAreaElement>('textarea[aria-label="Сообщение агенту"]');
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <div className="sidebar-course"><span className="eyebrow">УЧЕБНАЯ ЛАБОРАТОРИЯ</span><strong>Следующий этап.<br />По правилам.</strong><p>Утвердите план, выполните<br />и проверьте результат.</p></div>
      <nav className="main-nav" aria-label="Разделы лаборатории">
        ...
        <a href="#invariants"><Glyph kind="check" />Инварианты</a>
      ...
      <div className="sidebar-note"><span className="note-icon"><Glyph kind="book" size={18} /></span><strong>Нельзя перепрыгнуть этап</strong><p>Утверждение плана и проверка результата относятся к их актуальным версиям.</p></div>
      <div className="sidebar-bottom"><span className="local-dot" /><span>Локальная лаборатория<small>День 15 · Жизненный цикл</small></span></div>
    </aside>

    <main className="main-content" id="workspace">
      <header className="topbar"><span>Практика <span className="breadcrumb-slash">/</span> Архитектура агента</span><span className="day-pill">ДЕНЬ <b>15</b></span></header>
      <section className="page-heading">
        <div><div className="eyebrow teal-text">ПЕРЕХОДИТЬ ОСОЗНАННО</div><h1>Жизненный цикл задачи<span>.</span></h1><p>Сначала утверждение плана. Затем выполнение и проверка. Каждый переход обоснован.</p></div>
      ...
      {state?.workflow && <WorkflowPanel key={`${state.activeProfile}-${state.task.id}`} workflow={state.workflow} gates={state.gates} actionError={errorScope === 'transition' ? error : ''} disabled={busy} modelUnavailable={modelUnavailable} mode={mode} onModeChange={setMode} onAction={({ type, ...payload }) => action(type, payload)} onGenerate={kind => void generateWorkflow(kind)} />}

      {state?.invariants && <details className="invariants-fold"><summary><span>Инварианты проекта</span><strong>6 обязательных правил</strong><small>Архитектура, стек и бизнес-ограничения</small></summary><InvariantsPanel invariants={state.invariants} result={state.lastRun} disabled={busy} onProposal={changes => void checkProposal(changes)} onFillMessage={fillMessage} /></details>}
      ...
      <div className="workspace-grid">
        <div className="conversation-column">
          <section className="panel chat-panel" aria-labelledby="chat-heading">
            ...
            {state?.lastRun?.gate && !state.lastRun.gate.allowed && <div className="mode-notice chat-gate-notice"><strong>Действие пока недоступно.</strong> {state.lastRun.gate.reason}</div>}
      ...
      <footer className="page-footer"><span><span className="local-dot" />Данные хранятся локально · ключ доступен только серверу</span><span>День 15 / Контролируемые переходы</span></footer>
