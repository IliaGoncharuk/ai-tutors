...
import { createInitialState, view, applyAction, ask, compare, compareProfiles, generateWorkflow, checkProposal, buildContext, InputError } from './core.mjs';
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (url.pathname.startsWith('/api/')) {
        ...
        if (!['/api/action', '/api/chat', '/api/compare', '/api/profiles/compare', '/api/workflow', '/api/proposal'].includes(url.pathname)) throw new InputError('Маршрут не найден.', 404);
        ...
        try {
          ...
          else if (url.pathname === '/api/proposal') output = checkProposal(state, body);
