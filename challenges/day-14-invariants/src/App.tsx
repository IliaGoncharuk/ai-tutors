...
type Result = { text: string; usage?: Usage | null; context?: Context; mode?: ResultMode; label?: string; policy?: Policy };
type AppState = {
  day: number;
  title: string;
  revision: number;
...
export default function App() {
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <div className="sidebar-course"><span className="eyebrow">УЧЕБНАЯ ЛАБОРАТОРИЯ</span><strong>Свобода внутри<br />правил.</strong><p>Проверяйте решения<br />по принятым ограничениям.</p></div>
      ...
      <div className="sidebar-note"><span className="note-icon"><Glyph kind="book" size={18} /></span><strong>Помнить ограничения</strong><p>Принятое правило хранится отдельно от просьб в диалоге и проверяется перед ответом.</p></div>
      <div className="sidebar-bottom"><span className="local-dot" /><span>Локальная лаборатория<small>День 14 · Инварианты</small></span></div>
    </aside>

    <main className="main-content" id="workspace">
      <header className="topbar"><span>Практика <span className="breadcrumb-slash">/</span> Архитектура агента</span><span className="day-pill">ДЕНЬ <b>14</b></span></header>
      <section className="page-heading">
        <div><div className="eyebrow teal-text">РАБОТАТЬ В РАМКАХ ПРАВИЛ</div><h1>Инварианты агента<span>.</span></h1><p>Архитектура, стек и бизнес-правила. Агент учитывает их перед каждым ответом.</p></div>
      ...
      {state?.invariants && <InvariantsPanel invariants={state.invariants} result={state.lastRun} disabled={busy} onProposal={changes => void checkProposal(changes)} onFillMessage={fillMessage} />}

      {state?.workflow && <details className="workflow-fold"><summary><span>Состояние задачи</span><strong>{({ planning: 'Планирование', execution: 'Выполнение', validation: 'Проверка', done: 'Готово' })[state.workflow.stage]}</strong><small>{state.workflow.paused ? 'На паузе' : state.workflow.step}</small></summary><WorkflowPanel key={`${state.activeProfile}-${state.task.id}`} workflow={state.workflow} disabled={busy} modelUnavailable={modelUnavailable} mode={mode} onModeChange={setMode} onAction={({ type, ...payload }) => action(type, payload)} onGenerate={kind => void generateWorkflow(kind)} /></details>}
      ...
      <footer className="page-footer"><span><span className="local-dot" />Данные хранятся локально · ключ доступен только серверу</span><span>День 14 / Инварианты и ограничения</span></footer>
