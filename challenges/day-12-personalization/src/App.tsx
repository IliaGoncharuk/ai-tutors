...
export default function App() {
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <div className="sidebar-course"><span className="eyebrow">УЧЕБНАЯ ЛАБОРАТОРИЯ</span><strong>Один агент.<br />Разные люди.</strong><p>Исследуйте, как профиль<br />меняет привычный ответ.</p></div>
      ...
      <div className="sidebar-note"><span className="note-icon"><Glyph kind="book" size={18} /></span><strong>Учитывать автоматически</strong><p>Сохранённые предпочтения сопровождают каждый новый запрос.</p></div>
      <div className="sidebar-bottom"><span className="local-dot" /><span>Локальная лаборатория<small>День 12 · Персонализация</small></span></div>
    </aside>

    <main className="main-content" id="workspace">
      <header className="topbar"><span>Практика <span className="breadcrumb-slash">/</span> Архитектура агента</span><span className="day-pill">ДЕНЬ <b>12</b></span></header>
      <section className="page-heading">
        <div><div className="eyebrow teal-text">УЧИТЫВАТЬ ПОЛЬЗОВАТЕЛЯ</div><h1>Персональный агент<span>.</span></h1><p>Одна задача. Разные предпочтения. Ответ, который учитывает ваш профиль.</p></div>
      ...
      {state?.profile && <ProfilePanel profile={state.profile} profiles={state.profiles} disabled={busy} canCompare={canAsk} mode={mode} onAction={({ type, ...payload }) => action(type, payload)} onCompare={() => void compareProfiles()} />}
      ...
      <footer className="page-footer"><span><span className="local-dot" />Данные хранятся локально · ключ доступен только серверу</span><span>День 12 / Персонализация ассистента</span></footer>
