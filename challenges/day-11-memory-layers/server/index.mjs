...
import { createInitialState, view, applyAction, ask, compare, buildContext, InputError } from './core.mjs';
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (url.pathname.startsWith('/api/')) {
        ...
        if (!['/api/action', '/api/chat', '/api/compare'].includes(url.pathname)) throw new InputError('Маршрут не найден.', 404);
        ...
        try {
          ...
          else { if (!['live', 'demo'].includes(body.mode)) throw new InputError('Выберите live или demo.'); output = await (url.pathname === '/api/chat' ? ask : compare)(state, body, body.mode === 'live' ? generate : undefined); }
