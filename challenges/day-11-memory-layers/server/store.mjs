...
export class Repository {
  ...
  read() {
    ...
    const state = { ...read('system.json'), sessions: read('sessions.json'), tasks: read('tasks.json'), longTerm: read('long-term.json') };
    if (!state.sessions?.[state.activeSession] || !state.tasks?.[state.activeTask] || !Array.isArray(state.longTerm?.entries) || state.sessions[state.activeSession].taskId !== state.activeTask) throw new Error('Память повреждена; сохранённые файлы не перезаписаны.');
    return state;
  }
  write(state) {
    ...
    const { sessions, tasks, longTerm, ...system } = state;
    for (const [name, data] of Object.entries({ 'sessions.json': sessions, 'tasks.json': tasks, 'long-term.json': longTerm, 'system.json': system })) {
