...
export default function App() {
  ...
  const canAsk = !busy && !modelUnavailable && Boolean(message.trim());
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <div className="sidebar-course"><span className="eyebrow">УЧЕБНАЯ ЛАБОРАТОРИЯ</span><strong>От диалога<br />к памяти</strong><p>Исследуйте, что агент знает<br />и откуда он это знает.</p></div>
      ...
      <div className="sidebar-note"><span className="note-icon"><Glyph kind="book" size={18} /></span><strong>Один принцип</strong><p>Память становится понятной, когда у каждой записи есть своё место.</p></div>
      <div className="sidebar-bottom"><span className="local-dot" /><span>Локальная лаборатория<small>День 11 · Memory layers</small></span></div>
    </aside>

    <main className="main-content" id="workspace">
      <header className="topbar"><span>Практика <span className="breadcrumb-slash">/</span> Архитектура агента</span><span className="day-pill">ДЕНЬ <b>11</b></span></header>
      <section className="page-heading">
        <div><div className="eyebrow teal-text">ПОМНИТЬ ОСОЗНАННО</div><h1>Память агента<span>.</span></h1><p>Три слоя памяти. Один диалог. Вы решаете, что агент сохранит.</p></div>
      ...
      <section className="overview" aria-label="Как устроена память">
        ...
        <OverviewCard number="03" icon="book" name="Долговременная" caption="Профиль, решения и знания" count={longEntries.length} unit="записей" tone="amber" />
      </section>
      ...
      <div className="workspace-grid">
        <div className="conversation-column">
          ...
          {state?.lastRun?.context && <details className="panel context-panel"><summary><Glyph kind="eye" size={17} />Контекст последнего ответа<span>фактический запрос</span></summary><ContextView context={state.lastRun.context} /></details>}
      ...
      <footer className="page-footer"><span><span className="local-dot" />Данные хранятся локально · ключ доступен только серверу</span><span>День 11 / Модель памяти агента</span></footer>
...
function MemoryLayer({ title, subtitle, tone, icon, entries, empty, busy, onDelete }: { title: string; subtitle: string; tone: string; icon: string; entries: Entry[]; empty: string; busy: boolean; onDelete: (id: string) => void }) {
  return <section className="panel memory-layer"><div className="memory-heading"><span className={`layer-icon ${tone}`}><Glyph kind={icon} size={18} /></span><div><h3>{title}</h3><span>{subtitle}</span></div><b>{entries.length}</b></div>{entries.length === 0 ? <p className="memory-empty">{empty}</p> : <div className="memory-entries">{entries.map(entry => <article className="memory-entry" key={entry.id}><div className="entry-heading"><strong>{entry.key}</strong><button className="icon-button delete-button" disabled={busy} title={`Удалить «${entry.key}»`} aria-label={`Удалить «${entry.key}»`} onClick={() => onDelete(entry.id)}><Glyph kind="trash" size={15} /></button></div><p>{entry.value}</p>{entry.source && <span className="entry-source">Источник: {entry.source}</span>}</article>)}</div>}</section>;
}
